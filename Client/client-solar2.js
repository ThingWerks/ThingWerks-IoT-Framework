        ...
        inverter: [
            {
                ...
                floatRetrySun: true,            // optional - retry running inverter while floating based on sunlight name in solar.sun
    ...
    automation = {      // create an (name, clock) => {},  array member function for each automation 
        solar: function (autoName) {    // clock is declared every min.  time.sec time.min are local time, time.epoch and time.epochMin vars containing epoch time  
            ...
            if (!auto[autoName]) {
                ...
                setTimeout(() => {
                    setInterval(() => {
                        ...
                        if (cfg.solar.priority != undefined && cfg.solar.priority.haAuto
                            && entity[cfg.solar.priority.haAuto] && entity[cfg.solar.priority.haAuto].state == true)
                            priorityAuto();
                ...
                listeners();
                log("automation system started", autoName, 1);                 // log automation start with name number and severity 
                function initNV() {
                    nv.sensor ||= { watt: {}, kwh: {} };
                    ...
                    cfg.sensor.watt.forEach(config => {
                        if (nv.sensor.watt[config.name] == undefined) {
                            log("initializing NV memory for - watt sensor: " + config.name, autoName, 1);
                    ...
                    cfg.sensor.kwh.forEach(config => {
                        if (nv.sensor.kwh[config.name] == undefined) {
                            log("initializing NV memory for - kwh meter: " + config.name, autoName, 1);
                    ...
                    for (let x = 0; x < cfg.battery.length; x++)
                        if (nv.battery[x] == undefined || nv.battery[x].name != cfg.battery[x].name) {
                            log("initializing NV memory for - Battery: " + cfg.battery[x].name, autoName, 1);
                            nv.battery[x] = {
                ...
                if (cfg.solar.inverterAuto != undefined
                    && state.ha[cfg.solar.inverterAuto] == true) log("inverter automation is on", autoName, 1);
                else log("inverter automation is off", autoName, 1);
                file.write.nv();
            }
            //   checkWelder();
            /*
            ...
            */
            function priorityAuto() {
                ...
                if (!state.priority.boot) {
                    log("priority - system is going ONLINE", autoName, 1);
                ...
                function priorityCheck() {
                    ...
                    for (let x = 0; x < config.queue.length; x++) {         // when member is stopped
                        let member = state.priority.queue[x]; member.cfg = config.queue[x];
                        if (member.cfg.haAuto == undefined || member.cfg.haAuto != undefined && entity[member.cfg.haAuto].state == true) {
                            if (member.state == false || member.state == null) {
                                if (member.cfg.enable == true && member.cfg.onVolts != undefined) {
                                    if (member.cfg.inverter != undefined && state.inverter[member.cfg.inverter].state == true
                                        || member.cfg.inverter == undefined && state.inverter[0].state == true || member.cfg.inverter == undefined) {
                    ...
                    function trigger(x, message, newState) {
                        ...
                        if (member.delayStep == false) {
                            ...
                            log(message + member.cfg.name, autoName, 1);
                ...
                function pointers() {
                    ...
                    if (cfg.solar.espSunlight != undefined && entity[cfg.solar.espSunlight])
                        sun = Math.round(entity[cfg.solar.espSunlight].state * 100) / 100;
            ...
            function inverterAuto() {
                    ...
                    if (cfg.inverter[x].enable == true) {
                        ...
                        if (inverter.state == null || inverter.boot == false) {
                            if (cfg.inverter[x].power.ha != undefined) {
                                log("inverter: " + cfg.inverter[x].name + " - syncing HA power - state: " + state.ha[cfg.inverter[x].power.ha], autoName, 1);
                                if (state.ha[cfg.inverter[x].power.ha] == true) {
                                    if (state.esp[config.transfer.switchOn[0].id] != config.transfer.switchOn[0].state) {
                                        log("inverter: " + cfg.inverter[x].name + " - state ATS OFF (" + state.esp[config.transfer.switchOn[0].id]
                                            + ") is not expected - HA says its ON", autoName, 2);
                                    ...
                                    if (state.esp[config.transfer.switchOff[0].id] != config.transfer.switchOff[0].state) {
                                        log("inverter: " + cfg.inverter[x].name + " - state ATS ON (" + state.esp[config.transfer.switchOff[0].id]
                                            + ")is not expected - HA says its OFF", autoName, 2);
                            ...
                            if (inverter.state == false) {
                                if (config.power.startAuto == true) {
                                    if (voltsBat > config.voltsStop) {
                                        log("inverter: " + config.name + " - first start, volts is greater than stop volts, good to run", autoName, 1);
                                        inverterPower(x, true);
                                    }
                                } else if (nv.battery[config.battery].floating == true) {
                                    log("inverter: " + config.name + " - first start, battery is floating, good to run", autoName, 1);
                                    inverterPower(x, true);
                                }
                            }
                            inverter.boot = true;
                            ...
                            if (voltsInverter <= config.voltsFloatStop) {
                                log("inverter: " + config.name + " - inverter was faulted but now resetting because charger is no longer floating", autoName, 2);
                                inverter.state = false;
                            }
                            if (cfg.solar.watts.inverterPower != undefined
                                && state.sensor.watt[cfg.solar.watts.inverterPower] >= config.watts.welder) {
                                log("inverter: " + config.name + " - inverter was faulted but now resetting because welding is detected", autoName, 2);
                                inverter.state = false;
                            }
                        }
                    }
                        ...
                        if (inverter.state != "faulted" && inverter.state == true) {
                            if (inverterNightMode(x) == true) {
                                if (voltsBat < config.nightMode.startVoltageMin) {
                                    log("inverter: " + config.name + " - Battery is too low to make it through the night: " + voltsBat + ", switching off", autoName, 1);
                                    ...
                                    if (config.sunStop != undefined) {
                                        if (sun <= config.sunStop) {
                                            if (inverter.delaySwitchStep == false) {
                                                ...
                                                log("inverter: " + config.name + " - Sun is low: " + sun + "  voltsBat: " + voltsBat + ", switching to grid ", autoName, 1);
                                                inverterPower(x, false);
                                                return;
                                            }
                                    ...
                                    function ampStop() {
                                        if (inverter.delaySwitchStep == false) {
                                            ...
                                            log("inverter: " + config.name + " - solar power too low: " + ampsBat + "a  voltsBat: " + voltsBat
                                                + "v  Floating: " + nv.battery[config.battery].floating + " Sun: " + sun + "v  Watts: " + wattsInverter + ", switching to grid ", autoName, 1);
                                            inverterPower(x, false);
                            ...
                            if (voltsBat <= config.voltsStop) {  // when battery is low
                                if (inverter.delaySwitchVoltsStep == false) {
                                    ...
                                    log("inverter: " + config.name + " - Battery is low: " + voltsBat + ", is switching to grid ", autoName, 1);
                                    inverterPower(x, false);
                                    return;
                                }
                            ...
                            if (inverter.nightMode == true && time.hour >= config.nightMode.endHour
                                ...
                                if (config.nightMode.endAmps != undefined) {
                                    if (ampsBat >= config.nightMode.endAmps) {
                                        log("inverter: " + config.name + " - Battery is charging, current is high: " + ampsBat + "a  " + voltsBat + "v, exiting night mode", autoName, 1);
                            ...
                            if (voltsInverter < 20) {
                                if (inverter.delaySwitchFaultStep == false) {
                                    ...
                                    //   log("inverter: " + config.name + " has faulted - no output - Inverter Auto is shutting down ", autoName, 3);
                                    //    if (cfg.solar.inverterAuto != undefined) send(cfg.ha[cfg.solar.inverterAuto], false);
                                    log("inverter: " + config.name + " - FAULT - no output - Inverter is going offline ", autoName, 3);
                    ...
                    if (cfg.inverter[x].enable == true) {
                        ...
                        if (cfg.battery[config.battery].sensorVolt != undefined) {
                            if (inverter.state != "faulted" && inverter.state === false) {
                                ...
                                if (inverterNightMode(x) == true) {
                                    if (voltsBat >= config.nightMode.startVoltageMin) {
                                        log("Battery is enough to make it through the night: " + voltsBat + ", " + config.name + " switching on", autoName, 1);
                                        inverterPower(x, true);
                                    } else log("Battery is too low to make it through the night: " + voltsBat + ", " + config.name + " staying off", autoName, 1);
                                }
                                if (config.blackout == true && state.grid.state == false && voltsBat >= config.blackoutVoltMin) {
                                    log("Blackout detected, volts is enough to run: " + voltsBat + ", " + config.name + " switching on", autoName, 2);
                                    inverterPower(x, true);
                                }
                                if (config.power.id != undefined && state.esp[config.power.id] == false && inverter.delayOffTimer == null
                                    ...
                                    log("inverter: " + config.name + " is still on but should be off - manual power on??", autoName, 2);
                ...
                function checkCharging(x, power) {
                            ...
                            for (let y = 0; y < cfg.solar.sun.length; y++)
                                if (sun >= cfg.solar.sun[y].volts && wattsGrid < cfg.solar.sun[y].watts) {
                                    if (inverter.delaySwitchSunStep == false) {
                                        ...
                                        log("inverter: " + config.name + " - battery is floating, sun is enough: " + sun + "v  Power: " + wattsGrid + ", switching on", autoName, 1);
                                        finish();
                                        return;
                                    }
                                    match = true;
                        ...
                        if (config.sunRun != undefined) {
                            if (sun >= config.sunRun) {     // sunlight detection 
                                if (inverter.delaySwitchStep == false) {
                                    ...
                                    log("inverter: " + config.name + " - battery is charging, sun is high: " + voltsBat + ", switching on", autoName, 1);
                                    finish();
                                }
                            ...
                            if (ampsBat >= config.ampsRun) {   // current detection 
                                if (inverter.delaySwitchStep == false) {
                                    ...
                                    log("inverter: " + config.name + " - battery is charging, current is high: " + ampsBat + "a  " + voltsBat + "v, switching on", autoName, 1);
                                    finish();
                                }
                            ...
                            if (wattsSolar > ((wattsGrid * config.ampsStartGridMultiplier))) {
                                if (inverter.delaySwitchStep == false) {
                                    ...
                                    log("inverter: " + config.name + " - charge current is higher than grid power: " + wattsSolar + "w  " + voltsBat + "v, switching on", autoName, 1);
                                    finish();
                                }
                            }
                            ...
                            if (inverter.delaySwitchStep == false) {
                                ...
                                log("inverter: " + config.name + " - battery is charging: " + voltsBat + ", switching on", autoName, 1);
                                finish();
                            }
                        }
                    ...
                    function finish() {
                        if (inverter.nightMode == true) {
                            log("inverter: " + config.name + " - exiting night mode", autoName, 1);
                            inverter.nightMode = false;
                        }
                ...
                function pointers(x) {
                    ...
                    if (config.battery != undefined) {
                        if (cfg.battery[config.battery].sensorVolt != undefined)
                            voltsBat = Math.round(state.sensor.volt[cfg.battery[config.battery].sensorVolt] * 10) / 10;
                        if (cfg.battery[config.battery].sensorAmp != undefined) {
                            if (Array.isArray(cfg.battery[config.battery].sensorAmp)) {
                                let temp = 0;
                                for (let y = 0; y < cfg.battery[config.battery].sensorAmp.length; y++) {
                                    temp += parseFloat(state.sensor.amp[cfg.battery[config.battery].sensorAmp[y]]);
                                    ampsBat = Math.round(temp * 10) / 10;
                                }
                            } else ampsBat = Math.round(state.sensor.amp[cfg.battery[config.battery].sensorAmp] * 10) / 10;
                        }
                        if (cfg.battery[config.battery].sensorWatt != undefined)
                            wattsSolar = ~~parseFloat(state.sensor.watt[cfg.battery[config.battery].sensorWatt]);
                    }
            ...
            function inverterNightMode(x) {
                ...
                if (nightMode == true) {
                    if (state.inverter[x].nightMode == false) {
                        log("inverter: " + config.name + " - activating night mode", autoName, 1);
                    ...
                    for (let y = 0; y < listate.length; y++) {
                        setTimeout(() => {
                            log("inverter: " + config.name + " - intention: " + run + ", transferring ATS switch "
                                + y + " - state: " + list[y].state, autoName, 1);
                ...
                if (run == true) {
                    if (time.epoch - state.inverter[x].step >= cfg.solar.inverterRapidSwitch) {
                        if (config.power.id == undefined) {
                            log("inverter: " + config.name + " - transferring switches to inverter - wait", autoName, 1);
                            toggle(run);
                        } else {
                            if (state.esp[config.power.id] == false) {
                                send(cfg.esp[config.power.id], run);
                                log("inverter: " + config.name + " - power on, delaying transfer switch 10 secs", autoName, 1);
                                toggle(run);
                            } else {
                                log("inverter: " + config.name + " - already on - clearing shutdown delay", autoName, 1);
                        ...
                        log("inverter: " + config.name + " - cycling too frequently, epoch: "
                            + time.epoch + " step:" + state.inverter[x].step + " - inverter auto shutting down", autoName, 3);
                    ...
                    if (config.power.id == undefined) {
                        log("inverter: " + config.name + " - transferring switches to grid - wait", autoName, 1);
                        toggle(run);
                    } else {
                        log("inverter: " + config.name + " - transferring switches to grid - wait", autoName, 1);
                        toggle(run);
                        if (config.power.delayOff == undefined) {
                            log("inverter: " + config.name + ", power off", autoName, 1);
                            state.inverter[x].delayOffTimer = setTimeout(() => {
                                send(cfg.esp[config.power.id], run);
                                state.inverter[x].delayOffTimer = null;
                            }, 10e3);
                        } else {
                            log("inverter: " + config.name + " - shutdown will delay by:" + config.power.delayOff + " seconds", autoName, 1);
                            state.inverter[x].delayOffTimer = setTimeout(() => {
                                log("inverter: " + config.name + " - shutdown was delayed, now powering off", autoName, 1);
                                send(cfg.esp[config.power.id], run);
                                state.inverter[x].delayOffTimer = null;
                ...
                switch (fan.run) {
                    case false:
                        if (temp >= config.temp.on) {
                            log("room temp is rising, fan turning on: " + temp + tempUnit, autoName, 1);
                            fanSwitchNow();
                            fan.warn = true;
                            return;
                        }
                        if (config.sun != undefined) {
                            if (state.esp[cfg.solar.espSunlight] >= config.sun.on) {
                                if (fanSwitch(true)) {
                                    log("sun is high: " + state.esp[cfg.solar.espSunlight] + ", fan turning on: " + temp + tempUnit, autoName, 1);
                                    return;
                                }
                            ...
                            if (config.watts.night != undefined && state.inverter[0].nightMode == true) {
                                if (wattsBat >= config.watts.night.on) {
                                    if (fanSwitch(true)) {
                                        log("(nightmode) battery power is rising: " + wattsBat + "w - temp: " + temp + tempUnit + ", fan turning on ", autoName, 1);
                                        return;
                                    }
                                }
                                if (wattsInv >= config.watts.night.on) {
                                    if (fanSwitch(true)) {
                                        log("(nightmode) inverter power is rising: " + wattsInv + "w - temp: " + temp + tempUnit + ", fan turning on ", autoName, 1);
                                        return;
                                    }
                                }
                                ...
                                if (wattsBat >= config.watts.on) {
                                    if (fanSwitch(true)) {
                                        log("battery power is rising: " + wattsBat + "w - temp: " + temp + tempUnit + ", fan turning on ", autoName, 1);
                                        return;
                                    }
                                }
                                if (wattsInv >= config.watts.on) {
                                    if (fanSwitch(true)) {
                                        log("inverter power is rising: " + wattsInv + "w - temp: " + temp + tempUnit + ", fan turning on ", autoName, 1);
                                        return;
                                    }
                                }
                    ...
                    case true:
                        if (config.sun != undefined) {
                            if (state.esp[cfg.solar.espSunlight] <= config.sun.off) {
                                if (fanSwitch(false)) {
                                    log("room has cooled down and sun is low, fan turning off: " + temp + tempUnit, autoName, 1);
                                    return;
                                }
                            } else fan.delayStep = false;
                            ...
                            if (config.watts.night != undefined && state.inverter[0].nightMode == true) {
                                if (wattsBat <= config.watts.night.off) {
                                    if (fanSwitch(false)) {
                                        log("(nightmode) battery power has lowered: " + wattsBat + "w - temp: " + temp + tempUnit + ", fan turning off ", autoName, 1);
                                        return;
                                    }
                                }
                                if (wattsInv <= config.watts.night.off) {
                                    if (fanSwitch(false)) {
                                        log("(nightmode) inverter power has lowered: " + wattsInv + "w - temp: " + temp + tempUnit + ", fan turning off ", autoName, 1);
                                        return;
                                    }
                                }
                                ...
                                if (wattsBat <= config.watts.off) {
                                    if (fanSwitch(false)) {
                                        log("battery power has lowered: " + wattsBat + "w - temp: " + temp + tempUnit + ", fan turning off ", autoName, 1);
                                        return;
                                    }
                                }
                                if (wattsInv <= config.watts.off) {
                                    if (fanSwitch(false)) {
                                        log("inverter power has lowered: " + wattsInv + "w - temp: " + temp + tempUnit + ", fan turning off ", autoName, 1);
                                        return;
                                    }
                                }
                            ...
                            if (temp <= config.temp.off) {
                                if (fanSwitch(false)) {
                                    log("room has cooled, fan turning off: " + temp + tempUnit, autoName, 1);
                                    return;
                                }
                            } else fan.delayStep = false;
                        }
                    ...
                    case null:
                        ...
                        log("syncing fan state: " + fan.run, autoName, 1);
                        break;
                }
            ...
            function checkWelder() {    // needs to be rewritten for  cfg.inverter.sensorAmps or esp direct
                if (state.esp[cfg.inverter[0].espPower] >= cfg.inverter[0].welderWatts ||  // check inverter meter
                    ...
                    if (state.welder == false) {
                        log("welder detected: " + state.esp[cfg.inverter[0].espPower] + "w - shutting down all pumps", autoName, 2);
                        state.welder = true;
                    ...
                    if (state.welder == true && time.epoch - state.welderStep >= cfg.inverter[0].welderTimeout) {
                        log("welder not detected: " + state.esp[cfg.inverter[0].espPower] + "w", autoName, 1);
                        state.welder = false;
                    }
                }
            }
                ...
                for (let x = 0; x < cfg.battery.length; x++) {
                    ...
                    if (nv.battery[x].floating == null) {
                        if (volts <= config.voltsFullCharge && volts > config.voltsFloatStop) {
                            log("battery charger is floating: " + volts + "v", autoName, 1);
                    ...
                    if (nv.battery[x].floating == true) {       // consider blocking this until nighttime or some other criteria 
                        if (volts <= config.voltsFloatStop) {
                            if (bat.floatStep == false) {
                                ...
                                log("battery charger is no longer floating: " + volts + "v", autoName, 1);
                    ...
                    if (config.sensorWatt != undefined) {
                        ...
                        if (bat.min == null) {
                            log("starting recorder for battery - " + config.name, autoName, 1);
                            bat.min = time.min;
                        }
                        if (time.min != bat.min) {
                ...
                for (let x = 0; x < cfg.sensor.watt.length; x++) {
                    ...
                    if (config.record == true) {
                        ...
                        if (state.recorder.watt[config.name].min == null) {
                            log("starting recorder for watt sensor - " + config.name, autoName, 1);
            ...
            function calcSolarPower() {
                if (state.sunlight[sunConverted] == undefined) {
                    //   log("creating new solar power name: " + sunConverted, autoName, 1);
                    ...
                    if (solarPower < state.sunlight[sunConverted].low) {
                        ...
                        //     log("solar power name: " + sunConverted + "  low is being updated: " + solarPower + "  new average: " + state.sunlight[sunConverted].average, autoName, 1);
                    }
                    if (solarPower > state.sunlight[sunConverted].high) {
                        state.sunlight[sunConverted].high = solarPower;
                        state.sunlight[sunConverted].average = Math.round(state.sunlight[sunConverted].low + state.sunlight[sunConverted].high / 2);
                        //    log("solar power name: " + sunConverted + "  high is being updated: " + solarPower + "  new average: " + state.sunlight[sunConverted].average, autoName, 1);
                    }
                }
            }
            function listeners() {
                ...
                function sensors() {
                    cfg.sensor.amp.forEach((config, x) => {
                        ...
                        if (config.entity.length == 1) {
                            log("creating listener for Amp sensor: " + config.name, autoName, 1);
                    ...
                    cfg.sensor.volt.forEach((config, x) => {
                        ...
                        log("creating listener for Volt sensor: " + config.name, autoName, 1);
                    ...
                    cfg.sensor.watt.forEach((config, x) => {
                        ...
                        if (config.entity != undefined && config.entity.length == 1 && !config.solarPower) {
                            log("creating listener for Watt sensor: " + config.name, autoName, 1);
                            em.on(config.entity[0], (data) => {
                            ...
                            em.on(config.entity[0], (data) => {
                                ...
                                if (Number.isFinite(newData)) {
                                    if (Math.sign(newData) == -1) {
                                        //    log("Pzam KWh meter is negative - data: " + data + "  parsed data: " + newData, autoName, 2);
                                    } else {
                                        let diff = 0;
                                        // if (newData > 0 && newData <= 9999999) {
                                        if (newData > 0) {
                                            if (kwh.total == 0) {
                                                log("sensor: " + config.name + " first time recording", autoName, 1);
                                                kwh.total = newData;
                                            }
                                            if (state.sensor.kwh[config.name].last == 0) {
                                                log("starting recorder for PZEM Meter - " + config.name, autoName, 1);
                                                ...
                                                if (Math.sign(diff) == -1) {
                                                    ...
                                                    //            + "  old NV reading: " + kwh.total + "  diff: " + diff + "  raw: " + data, autoName, 0);
                                                } else {
                                                    recorder(entity, diff, config.name);
                                                    if (diff > 0) kwh.total += diff;
                                                    // log("Pzam KWh meter: " + x + " difference is positive - parsed data: " + newData
                                                    //     + "  old NV reading: " + nv.sensor.kwh[x].total + "  diff: " + diff + "  raw: " + data, autoName, 0);
                                                }
                                            }
                                        }
                                    }
                ...
                if (cfg.solar.inverterAuto != undefined)
                    em.on(cfg.solar.inverterAuto, (newState) => {
                        if (newState == true) log("inverter auto going online", autoName, 1);
                        else log("inverter auto going offline", autoName, 1);
                    });
                    ...
                    em.on(cfg.ha[cfg.inverter[x].power.ha], (newState) => {   // keep inverter state synced to automation state
                        if (cfg.inverter[x].enable)
                            if (state.inverter[x].state != "faulted" && state.inverter[x].state != newState) {
                                log("inverter: " + cfg.inverter[x].name + " - HA power state switching to " + newState, autoName, 1);
                ...
                if (cfg.solar.priority.haAuto != undefined)
                    em.on(cfg.solar.priority.haAuto, (newState) => {
                        if (newState == true) log("priority auto going online", autoName, 1);
                        else log("priority auto going offline", autoName, 1);
                        state.priority.step = time.epoch;
                    });
                for (let x = 0; x < cfg.solar.priority.queue.length; x++) {
                    ...
                    if (queue.haAuto != undefined)
                        em.on(queue.haAuto, (newState) => {
                            if (newState == true) log("priority queue member" + queue.name + " going online", autoName, 1);
                            else log("priority queue member " + queue.name + " going offline", autoName, 1);
                        });
                    em.on(queue.devices[0], (newState) => {
                        log("syncing priority queue: " + queue.name + "  with new entity state: " + newState, autoName, 1);
                    ...
                    log("syncing priority queue: " + queue.name + "  with entity: " + queue.devices[0]
                        + " - state: " + entity[queue.devices[0]].state, autoName, 1);
                    state.priority.queue[x].state = entity[queue.devices[0]].state;
                    state.priority.step = time.epoch;
                }
            }
            function timer() {
