    ...
    automation: {
        Pumper: function (_name, _push, _reload) {
            ...
            let
                                        ...
                                        switch (dd.fault.flow) {
                                            case false: // when pump is STOPPED and not flow faulted
                                                ...
                                                if (dd.state.timeoutOff == true) {  // after pump has been off for a while
                                                    if (dd.pump[dd.state.pump].state === true && dd.shareddelivery.run == false) {
                                                        ...
                                                        if (dd.flow != undefined && dd.flow[dd.state.pump].lm > dd.cfg.pump[dd.state.pump].flow.startError
                                                            && dd.warn.flowFlush == false && dd.shareddelivery.shared == false) {
                ...
                pump = {
                    run: function (name, intention, requestor) {
                        st.pump[name] ||= {
                            run: false,
                            cycle: 0, cycleLast: null,
                            timeStart: null,
                            retryCount: 0,
                            timer: { flowStart: null, flowRestart: null, run: null },
                            queue: {},
                        }
                        state = st.pump[name];
                        config = cfg.pump[name];
                        timeStart = time.epoch;

                        if (intention) {
                            if (config.cycleMax && config.cyclePeriod) {
                                if (state.cycle <= config.cycleMax) startType();
                                else {
                                    if ((time.epoch - state.cycleLast) < config.cyclePeriod) {
                                        log("pump - " + name + " - faulted - pump over cycling - cycled: " + (state.cycle + 1)
                                            + " in: " + (time.epoch - state.cycleLast) + " seconds");
                                        return;
                                    }
                                }
                            } else startType();
                        ...
                        function startType() {
                            if (intention.type) {
                                ...
                                log("pump - " + name + " - starting - basic run - requested by - " + requestor);
                                state.queue[requestor] = { type: "none" };
                        ...
                        function start() {
                            ...
                            if (config.flow?.entity)
                                state.timer.flowStart = state.timer.flowStart = setTimeout(() => {
                                    startCheckFlow();
                                }, (config.flow.startCheckDelay * 1e3));
                        }
                        function startCheckFlow() {
                            if (config.flow?.entity) {
                                flow = entity[config.flow?.entity].lm
                                log("pump - " + name + " - checking flow");
                                if (flow <= config.flow.startError) {
                                    state.run = "failed";
                                    if (config.flow.retry) {
                                        if ((state.retryCount + 1) < config.flow.retryTimes) {
                                            log("pump - " + name + " - start flow check FAILED - flow: " + flow + " lm - attempt: " +
                                                +(state.retryCount + 1) + " - retry in: " + config.flow.retryWait + "s", 2);
                                            stop();
                                            state.retryCount++;
                                            state.timer.flowRestart = setTimeout(() => {
                                                log("pump - " + name + " - restart delay complete - attempt: "
                                                    + (state.retryCount + 1) + " - starting again");
                                                start();
                                            }, (config.flow.retryWait * 1e3));
                                        } else if ((state.retryCount) < config.flow.retryTimes) {
                                            log("pump - " + name + " - start flow check FAILED - flow: " + flow + " lm - attempt: " +
                                                +(state.retryCount + 1) + " - final retry in: " + config.flow.retryWaitFinal + "s", 2);
                                            stop();
                                            state.retryCount++;
                                            state.timer.flowRestart = setTimeout(() => {
                                                log("pump - " + name + " - final restart delay complete - attempt: "
                                                    + (state.retryCount + 1) + " - last attempt - starting again");
                                                start();
                                            }, (config.flow.retryWaitFinal * 1e3));
                                        } else {
                                            log("pump - " + name + " - start flow check FAILED - flow: " + flow + " lm - attempt: " +
                                                +(state.retryCount + 1) + " - retry in: " + config.flow.retryWait + "s", 2);
                                            stop();
                                        ...
                                        log("pump - " + name + " - start flow check FAILED - flow: " + flow + " lm", 3);
                                        stop();
                                    }
                                } else if (flow <= config.flow.startError) { // pump running successfully   
                                    log("pump - " + name + " - starting flow is lower than expected - flow: " + flow + " lm", 2);
                                    state.timer.run = setInterval(() => { run(); }, 1e3);
                                } else {
                                    log("pump - " + name + " - starting flow check PASSED - flow: " + flow + " lm", 1);
                                    state.timer.run = setInterval(() => { run(); }, 1e3);
                                }
                            }
                        }
                        ...
                        function stop() {
                            log("pump - " + name + " - stopping", 1);
                            state.cycle++;
                            clearInterval(state.timer.run);
                            clearTimeout(state.timer.flowStart);
                            clearTimeout(state.timer.flowRestart);
            ...
            if (_push === "init") {
                ...
                cfg.fountain?.forEach(element => {
                    ...
                    push[element.haAuto] = push_constructor.fountain();
                });
                cfg.pump?.forIn((name, value) => {
                    st.pump.name = {

                    }
                })

                cfg.dd?.forEach(demandDelivery => {
                    log(demandDelivery.name + " - initializing");
                    st.dd.push({     // initialize volatile memory for each Demand Delivery system
                        ...
                        auto: { get state() { return entity[demandDelivery.ha.auto]?.state }, name: demandDelivery.ha.auto },
                        press: {
                            in: (demandDelivery.press.input != undefined && demandDelivery.press.input.sensor) ? {
                                cfg: cfg.sensor.press.find(obj => obj.name === demandDelivery.press.input.sensor),
                                get state() { return entity[("press_" + demandDelivery.press.input.sensor)] }
                            } : undefined,
                            out: (demandDelivery.press.output != undefined && demandDelivery.press.output.sensor) ? {
                                cfg: cfg.sensor.press.find(obj => obj.name === demandDelivery.press.output.sensor),
                                get state() { return entity[("press_" + demandDelivery.press.output.sensor)] },
                            } : undefined,
                        },
                        pump: [],
                    })
                    for (let y = 0; y < demandDelivery.pump.length; y++) {
                        st.dd[x].pump.push({
                            cfg: demandDelivery.pump[y],
                            get state() { return entity[demandDelivery.pump[y].entity]?.state; },
                            name: demandDelivery.pump[y].entity
                        });
                        if (demandDelivery.pump[y].flow != undefined & demandDelivery.pump[y].flow.sensor != undefined) {
                            st.dd[x].flow ||= [];
                            st.dd[x].flow.push(entity[("flow_" + demandDelivery.pump[y].flow.sensor)])
                        }
                    }
                    if (demandDelivery.press != undefined && demandDelivery.press.output != undefined
                        && demandDelivery.press.output.profile != undefined) {
                        if (demandDelivery.ha.profile != undefined && demandDelivery.press.output.profile.length > 0) {
                            log(demandDelivery.name + " - profile selector enabled");
                            st.dd[x].state.profile = parseInt(entity[demandDelivery.ha.profile]?.state);
                        } else {
                            log(demandDelivery.name + " - no profile entity - selecting profile 0");
                            st.dd[x].state.profile = 0;
                        }
                    }
                    if (demandDelivery.ha.turbo != undefined) {
                        st.dd[x].state.turbo = entity[demandDelivery.ha.turbo].state;
                    }
                });
